...
| Mode-2 adaptive trim on 40032 (replaces per-tick cutoff write) | Mode 2's cascade is `PV → house → battery (up to 40032) → export → curtail`. With `40032=max`, the battery saturates before any export flows, killing the daytime split. Adaptive dispatch writes 40032=max + mode 2, sleeps 5 s, reads measured surplus, then writes `40032 = max(LP_rate, surplus − export_cap − headroom)` so the cascade splits cleanly. The earlier "mode 2 + per-tick cutoff = soc_pct_end" design (shipped acea3f5, retired 1f363a7) is gone — `probe_no_cutoff.py` confirmed 40032 alone is a sufficient rate knob, and pinning cutoff at the ceiling avoids interaction with periodic SOC re-assertion. | clients/sigenergy.py `_apply_mode2_adaptive_charge`, lp/dispatch.py, probe_two_phase.py, probe_no_cutoff.py |
| Idle dispatch via `40032=0` | Mode-2 idle writes `40032=0, mode=2`. The cascade's battery leg respects 40032 (verified empirically), so no PV-to-battery charge; surplus exports up to DNSP. Discharge to cover load shortfall is unaffected (40032 caps charge only; discharge floor is 40048). Replaces the older `cutoff = current_soc + buffer` idle convention. | clients/sigenergy.py `apply_lp_dispatch`, probe_no_cutoff.py P1 |
