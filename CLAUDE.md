...
| Wear cost lifted 2.5 → 5 c/kWh one-way (10 c/kWh round-trip) | 2026-04-25 overnight LP cycled 22 kWh at 8 c export, planning to refill from "free" PV the next morning. At wear=2.5 the round-trip cost was 5 c/kWh, so any export above 2.5 c looked profitable in the LP's model. Per-tick diagnostic at the failure moment (Sat 23:00 AEST, SOC=50%, ep=8 c, ip=23 c): wear=2.5 → discharge -6 kW exporting 5 kW; wear=4.5+ → drop to -1 kW (house only). 5 c is halfway to the "true" LFP wear (~10 c/kWh per CLAUDE.md sizing) and break-even spread now ~13 c rather than ~7.8 c — keeps real evening-peak arbitrage but suppresses speculative mid-tier export. Validated via closed-loop simulator (`simulate.py` + `simulate_sweep.py`) with synthetic adverse scenarios (PV-bust 50% / 30%, forecast-too-rosy). | lp/constants.py `WEAR_COST_PER_KWH` |
